...
class PaymentProducer(threading.Thread, PaymentProducerABC):
                ...
                if (
                    ...
                    if not self.payments_queue.full():
                        ...
                        if level_in_cycle < self.payment_offset:
                            ...
                            result = self.try_to_pay(pymnt_cycle, self.rewards_type, self.nw_config, current_cycle)
    ...
    def compute_rewards(self, pymnt_cycle, computation_type, network_config, adjustments = {}):
        '''
        ...
        reward_model = self.reward_api.get_rewards_for_cycle_map(pymnt_cycle, computation_type)
    ...
    def recompute_rewards(self, completed_cycle, computation_type, network_config):
        ...
        calculation csv file and return an adjustment map.
        ...
        '''
        logger.info("Checking for potential adjustment for recently completed cycle {:s}.".format(str(completed_cycle)))
        completed_cycle_report_file_path = get_calculation_report_file(self.calculations_dir, completed_cycle)
        adjustments = {}
        if os.path.isfile(completed_cycle_report_file_path):
            logger.info("TRD ran for cycle: {:s}, calculating adjustments.".format(str(completed_cycle)))
            reward_logs_from_report, total_amount_from_report, rewards_type_from_report = CsvCalculationFileParser().parse(completed_cycle_report_file_path, self.baking_address)
            # check that the overestimate has not been computed yet
            if sum([rl.overestimate for rl in reward_logs_from_report]) > 0:
                logger.info("Overestimate has already been calculated for cycle {:s}, not calculating it again.".format(str(completed_cycle)))
                completed_cycle_reward_logs, completed_cycle_total_amount = reward_logs_from_report, total_amount_from_report
            else:
                completed_cycle_reward_logs, completed_cycle_total_amount = \
                   self.compute_rewards(completed_cycle, computation_type, network_config)
            overestimate = total_amount_from_report - completed_cycle_total_amount
            logger.info("We {:s}estimated payout for cycle {:s} by {:,} mutez, will attempt to adjust.".format(("over" if overestimate >0 else "under"), str(completed_cycle), abs(overestimate)))
            for rl in completed_cycle_reward_logs:
                rl.overestimate = int(Decimal(rl.ratio * overestimate).to_integral_value(rounding=ROUND_HALF_DOWN))
                ...
                logger.debug(f"Will try to recover {adjustments[rl.address]} mutez for {rl.address} based on past overpayment")

            CsvCalculationFileParser().write(completed_cycle_reward_logs, completed_cycle_report_file_path, completed_cycle_total_amount, rewards_type_from_report, self.baking_address, False)
        return adjustments
    ...
    def try_to_pay(self, pymnt_cycle, rewards_type, network_config, current_cycle):
        try:
            ...
            if self.release_override == -11 and pymnt_cycle >= current_cycle:
                ...
                adjustments = self.recompute_rewards(completed_cycle, rewards_type, network_config)
            ...
            reward_logs, total_amount = \
                   self.compute_rewards(pymnt_cycle, current_cycle_rewards_type, network_config, adjustments)
            ...
            if total_adjustments_to_recover > 0:
                logger.debug("Total adjustments to recover is {:,}, total recovered adjustment is {:,}.".format(total_adjustments_to_recover, total_recovered_adjustments))
                logger.info("After early payout of cycle {:s}, {:,} mutez were not recovered.".format(str(completed_cycle), total_adjustments_to_recover + total_recovered_adjustments))
            ...
            CsvCalculationFileParser().write(reward_logs, report_file_path, total_amount, current_cycle_rewards_type, self.baking_address, early_payout)

            # 4- calcylate total amount
            for rl in reward_logs:
                rl.cycle = pymnt_cycle
            total_amount_to_pay = sum([rl.adjusted_amount for rl in reward_logs if rl.payable])
