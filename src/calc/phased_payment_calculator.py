...
class PhasedPaymentCalculator:
    ...
    def calculate(self, reward_provider_model, adjustments = None):
        ...
        rwrd_logs, total_rwrd_amnt = phase_last.calculate(rwrd_logs, total_rwrd_amnt, adjustments)
        ...
        total_delegator_amounts = sum([rl.adjusted_amount for rl in rwrd_logs if not rl.skipped])
        total_adjustments = sum([rl.adjustment for rl in rwrd_logs if not rl.skipped])
        amnt_pay_diff = abs(total_rwrd_amnt + total_adjustments - total_delegator_amounts)

        logger.info("Total rewards after processing is {:,} mutez.".format(total_rwrd_amnt))
        if total_adjustments < 0:
            logger.info("Total adjustment for past early payout is {:,} mutez.".format(total_adjustments))
            logger.info("Adjusted total rewards is {:,} mutez.".format(total_rwrd_amnt + total_adjustments))
        logger.info("Sum of amounts allocated to delegators is {:,} mutez".format(total_delegator_amounts))
        logger.info("Difference between total rewards and sum of amounts allocated to delegators is {:,} mutez. "
